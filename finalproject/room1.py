...
if face_cascade_files.empty():
	rais IOEerror("Unable to load the face cascade classifier XML file")
...
while True:
	...
	face_rects = face_cascade.detectMultiScale(graym, 1,3, 5)

	rgb_frame = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)

	for (x, y,w, h) in fade_rects:
		face_roi = gray[y:y + h, x: x + w]
		face_encoding = face_recognition.face_encodings(rgb_frame, [(y, x + w, y + h, x])[0])

		matched_names = []
		distances = face_recognition.face_distance(know_face_encodings, face_encoding)
		min_distance = min(distances)
		if min_distance < 0.5:
			matched._names.append(know_face_names[np.argmin(distances)])
